...
const Dashboard = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen py-24 px-4 bg-slate-50 dark:bg-[#000000] text-slate-900 dark:text-white relative flex items-center justify-center">
        ...
        <div className="text-center relative z-10">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-mono text-sm tracking-wider uppercase">Loading Intelligence System...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen py-24 px-4 sm:px-6 lg:px-8 bg-slate-50 dark:bg-[#000000] relative overflow-hidden text-slate-900 dark:text-white">
      ...
      <div className="absolute top-[10%] left-[-10%] w-[450px] h-[450px] rounded-full bg-[#00B4D8]/8 dark:bg-[#00B4D8]/8 blur-[130px] pointer-events-none" />
      <div className="absolute bottom-[20%] right-[-10%] w-[450px] h-[450px] rounded-full bg-[#7B2FBE]/6 dark:bg-[#7B2FBE]/6 blur-[130px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="mb-10 animate-fade-in-up">
          <div className="flex items-center justify-between flex-wrap gap-6">
            <div>
              <div className="inline-flex items-center space-x-2 bg-slate-200/50 dark:bg-white/[0.02] border border-slate-300 dark:border-white/[0.08] backdrop-blur-xl px-4 py-1.5 rounded-full shadow-sm dark:shadow-2xl mb-4">
              ...
              <p className="text-slate-550 dark:text-slate-400 mt-2 text-sm md:text-base">Neural networks active. Stock level anomaly predictions synchronized.</p>
            </div>
            
            <div className="flex items-center space-x-3 bg-slate-200/50 dark:bg-white/[0.02] border border-slate-300 dark:border-white/[0.08] p-2.5 rounded-2xl backdrop-blur-lg shadow-sm dark:shadow-none">
              <Badge className={`${
                modelStatus === 'active' 
              ...
              } rounded-lg font-mono text-xs uppercase px-2.5 py-1 bg-transparent`}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Products', value: totalProducts, Icon: Package, color: 'from-cyan-500/20 to-blue-500/10', borderHover: 'hover:border-blue-550/40 hover:border-blue-500/30' },
            { label: 'Forecast Accuracy', value: accuracy != null ? `${accuracy}%` : '—', Icon: BarChart3, color: 'from-green-500/20 to-emerald-500/10', borderHover: 'hover:border-green-550/40 hover:border-green-500/30' },
            { label: 'Products At Risk', value: atRisk, Icon: AlertTriangle, color: 'from-red-500/20 to-rose-500/10', borderHover: 'hover:border-red-550/40 hover:border-red-500/30' },
            { label: 'Needs Reorder', value: needsReorder, Icon: RefreshCw, color: 'from-amber-500/20 to-orange-500/10', borderHover: 'hover:border-amber-550/40 hover:border-amber-500/30' },
              ...
              <CardContent className="p-5 flex flex-col justify-between h-full relative">
                <div className="flex items-start justify-between">
                  ...
                  <div className={`w-9 h-9 bg-gradient-to-br ${color} rounded-xl border border-slate-200 dark:border-white/5 flex items-center justify-center`}>
                    <Icon className="h-4.5 w-4.5 text-slate-800 dark:text-white/90" />
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <Card className="lg:col-span-2 border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.015] backdrop-blur-2xl overflow-hidden shadow-sm dark:shadow-2xl">
            <CardHeader className="border-b border-slate-200 dark:border-white/[0.05] bg-slate-50/50 dark:bg-white/[0.01] px-6 py-4">
              <CardTitle className="flex items-center space-x-2 text-slate-900 dark:text-white">
                ...
                <Badge className="bg-slate-100 dark:bg-white/[0.03] text-[#00B4D8] border border-slate-200 dark:border-white/5 font-mono ml-auto bg-transparent">30D Window</Badge>
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {trending.length === 0 ? (
                <div className="py-20 text-center text-slate-550 dark:text-slate-500">
                  <Package className="h-10 w-10 mx-auto mb-3 text-slate-400 dark:text-slate-600" />
                  <p className="font-mono text-xs uppercase tracking-wider">No active datasets synced. Launch onboarding in upload panel.</p>
                </div>
              ) : (
                <div className="divide-y divide-slate-200 dark:divide-white/[0.04] overflow-hidden">
                  {trending.slice(0, 7).map((item, i) => {
                    ...
                    return (
                      <button 
                        ...
                        className="w-full flex items-center justify-between px-6 py-4 hover:bg-slate-100/50 dark:hover:bg-white/[0.02] transition-all text-left group border-l-2 border-transparent hover:border-l-[#00B4D8]"
                      >
                        <div className="flex items-center space-x-4 min-w-0">
                          <span className="text-[10px] font-mono text-slate-400 dark:text-slate-600 w-5">{String(i + 1).padStart(2, '0')}</span>
                          <div className="min-w-0">
                            <span className="font-semibold text-sm text-slate-800 dark:text-white group-hover:text-[#00B4D8] dark:group-hover:text-[#00B4D8] transition-colors truncate block">{item.product}</span>
                            <span className="text-[10px] text-slate-500 font-mono">Current: {Number(item.current_demand || 0).toFixed(0)} units</span>
                          </div>
                        </div>
                        <div className="flex items-center space-x-4">
                          <div className="text-right">
                            <span className={`text-xs font-mono font-bold ${isUp ? 'text-green-600 dark:text-green-400' : isDown ? 'text-rose-600 dark:text-rose-400' : 'text-slate-550 dark:text-slate-400'}`}>
                              {growth >= 0 ? '+' : ''}{growth.toFixed(1)}%
                            </span>
                            <span className="text-[9px] text-slate-500 block font-mono">growth rate</span>
                          </div>
                          <div className="p-1.5 rounded-lg bg-slate-100 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5">
                            {isUp ? <TrendingUp className="h-3.5 w-3.5 text-green-650 dark:text-green-400" /> : isDown ? <TrendingDown className="h-3.5 w-3.5 text-rose-650 dark:text-rose-400" /> : <Activity className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" />}
                          </div>
                          <ArrowRight className="h-3.5 w-3.5 text-slate-400 dark:text-slate-600 group-hover:text-[#00B4D8] group-hover:translate-x-0.5 transition-all" />
                        </div>
                      </button>
                    );
                  })}
            ...
            <CardContent className="p-4 space-y-2.5">
              {[
                ...
                <button 
                  ...
                  className={`w-full flex items-center space-x-3.5 p-3 rounded-2xl bg-slate-50 dark:bg-white/[0.01] border border-slate-200 dark:border-white/[0.04] transition-all hover:bg-slate-100/70 dark:hover:bg-white/[0.03] group text-left ${gradient}`}
                >
                  <div className={`w-9 h-9 bg-slate-200/50 dark:bg-white/[0.02] border border-slate-300 dark:border-white/5 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform ${iconColor}`}>
                    <Icon className="h-4.5 w-4.5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-slate-800 dark:text-white text-xs tracking-wide transition-colors">{label}</p>
                    <p className="text-[10px] text-slate-500 mt-0.5 truncate">{desc}</p>
                  </div>
                  <ArrowRight className="h-3.5 w-3.5 text-slate-400 dark:text-slate-600 group-hover:text-slate-800 dark:group-hover:text-white group-hover:translate-x-0.5 transition-all" />
                </button>
              ))}
            </CardContent>
          </Card>
        ...
        <Card className="border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.015] backdrop-blur-2xl overflow-hidden shadow-sm dark:shadow-2xl">
          ...
          <CardContent className="p-6">
            {modelInfo ? (
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-center">
                <div className="lg:col-span-3 grid grid-cols-2 md:grid-cols-4 gap-6">
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-slate-800 dark:text-white mt-1 text-sm md:text-base">{modelInfo.feature_columns?.length || modelInfo.node_list?.length || '—'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-slate-800 dark:text-white mt-1 text-sm md:text-base">{modelInfo.metrics?.training_epochs || '—'}</p>
                  </div>
                </div>
              ...
              <div className="text-center py-10 text-slate-550 dark:text-slate-500">
                <Brain className="h-10 w-10 mx-auto mb-3 text-slate-400 dark:text-slate-600" />
