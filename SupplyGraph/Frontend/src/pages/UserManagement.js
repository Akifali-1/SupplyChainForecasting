...
const UserManagement = () => {
  ...
  return (
    <div className="min-h-screen bg-[#000000] text-slate-100 relative overflow-hidden font-sans pb-16">
      {/* Ambient Glows */}
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8 z-10 w-full relative">
        {/* Header Banner */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0 border-b border-white/[0.08] pb-6">
          <div>
            ...
            <p className="text-slate-400 mt-1 text-sm">
              ...
              <span className="font-semibold text-indigo-300">{user?.companyName}</span>.
            </p>
          </div>
        ...
        {inviteUrl && (
          <Card className="border border-indigo-500/30 bg-indigo-500/5 backdrop-blur-xl rounded-2xl shadow-[0_0_30px_rgba(99,102,241,0.1)] p-6 relative overflow-hidden transition-all duration-300">
            <CardHeader className="pb-2 p-0">
              <CardTitle className="text-indigo-400 text-lg flex items-center space-x-2">
            ...
            <CardContent className="p-0 pt-4">
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1 bg-white/[0.02] border border-white/[0.08] px-4 py-3 rounded-xl font-mono text-sm text-indigo-200 break-all select-all flex items-center">
                  {inviteUrl}
                </div>
                <Button
                  variant="outline"
                  onClick={handleCopyLink}
                  className="flex items-center space-x-2 border-indigo-500/30 hover:bg-indigo-500/20 hover:border-indigo-500/50 text-indigo-300 hover:text-indigo-200 font-semibold h-auto py-3 px-5 rounded-xl bg-transparent transition-all duration-200"
                >
        ...
        <Card className="border border-white/[0.07] bg-white/[0.015] backdrop-blur-2xl rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden">
          ...
          <CardContent className="p-6">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 space-y-4 border-2 border-dashed border-white/[0.08] bg-white/[0.005] rounded-2xl">
                <div className="w-12 h-12 rounded-full bg-slate-500/10 border border-slate-500/20 flex items-center justify-center">
                  <AlertCircle className="h-6 w-6 text-slate-400" />
                </div>
                <p className="font-semibold text-slate-300">No teammates found</p>
                  ...
                  <tbody className="divide-y divide-white/[0.04] text-sm">
                    {members.map((member) => {
                      ...
                      return (
                        <tr
                          key={member._id}
                          className="hover:bg-white/[0.015] transition-all duration-200"
                        >
                          <td className="py-4 px-4 font-semibold text-white flex items-center space-x-3">
                            <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-indigo-500/20 to-purple-500/20 border border-white/[0.1] flex items-center justify-center text-sm font-bold font-mono text-indigo-300 shadow-inner">
                              {member.name ? member.name[0].toUpperCase() : "?"}
                            </div>
                            <span className="flex items-center">
                              {member.name || "Unnamed User"}
                              {isSelf && (
                                <span className="ml-2 text-xs font-semibold px-2 py-0.5 rounded bg-indigo-500/10 border border-indigo-500/20 text-indigo-300">
                                  You
                                </span>
                              )}
                            </span>
                          ...
                          <td className="py-4 px-4">
                            {isAdmin ? (
                              <span className="bg-purple-500/10 border border-purple-500/20 text-purple-300 px-2.5 py-0.5 rounded-full text-xs font-bold uppercase inline-flex items-center">
                                <Shield className="h-3 w-3 mr-1" />
                                <span>{member.role}</span>
                              </span>
                            ) : (
                              <span className="bg-white/[0.04] border border-white/[0.08] text-slate-300 px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase inline-flex items-center">
                                <span>{member.role}</span>
                              </span>
                            )}
                          </td>
                          ...
                          <td className="py-4 px-4 text-right">
                            {!isSelf && !isAdmin && (
                              <Button
                                ...
                                className="text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 border border-rose-500/20 rounded-xl px-4 py-2 flex items-center space-x-1.5 transition-all duration-200 ml-auto bg-transparent"
                              >
