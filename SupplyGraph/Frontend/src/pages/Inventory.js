...
const Inventory = () => {
  ...
  const ActionBadge = ({ action }) => {
    const cfg = {
      'Stock Up': { cls: 'bg-green-500/10 text-green-400 border border-green-500/20', Icon: ArrowUpCircle },
      'Reduce': { cls: 'bg-red-500/10 text-red-400 border border-red-500/20', Icon: ArrowDownCircle },
      'Maintain': { cls: 'bg-white/[0.02] text-slate-400 border border-white/10', Icon: MinusCircle },
    };
      ...
      <div className="max-w-7xl mx-auto relative z-10 animate-fade-in">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
                  ...
                  >
                    ...
                    <div className="p-6">
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-4 font-mono text-xs">
                        <div className="bg-white/[0.01] border border-white/[0.04] rounded-xl p-3 text-center">
                          <p className="text-[9px] uppercase tracking-wider text-slate-500 font-semibold mb-1">Current</p>
                          <p className="text-base font-bold text-white">{d.current.toLocaleString()}</p>
                        </div>
                        <div className={`border rounded-xl p-3 text-center bg-white/[0.01] ${isDown ? 'border-red-500/10' : isUp ? 'border-green-500/10' : 'border-slate-500/10'}`}>
                          <p className="text-[9px] uppercase tracking-wider text-slate-500 font-semibold mb-1">Predicted</p>
                          <p className={`text-base font-bold ${isDown ? 'text-rose-400' : isUp ? 'text-green-400' : 'text-[#00B4D8]'}`}>{d.predicted.toLocaleString()}</p>
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        <div className="flex items-center justify-between text-[10px] font-mono mb-1.5">
                          ...
                          <span className={`font-bold ${isDown ? 'text-rose-400' : isUp ? 'text-green-400' : 'text-slate-400'}`}>
                            {pctChange >= 0 ? '+' : ''}{pctChange.toFixed(1)}%
                          </span>
                        </div>
                        <div className="h-1.5 bg-white/[0.04] rounded-full overflow-hidden relative">
                          <div
                            className={`h-full rounded-full transition-all duration-500 bg-gradient-to-r ${isDown ? 'from-red-500 to-orange-400' : isUp ? 'from-green-500 to-emerald-400' : 'from-[#00B4D8] to-[#7B2FBE]'}`}
                            style={{ width: `${barWidth}%` }}
                          />
                        </div>
                      </div>

                      <div className="flex items-center justify-between pt-3 border-t border-white/[0.04] h-8">
          ...
          <TabsContent value="optimization" className="space-y-6 animate-fade-in-up">
            <Card className="border border-white/[0.06] bg-white/[0.015] backdrop-blur-2xl shadow-2xl rounded-2xl overflow-hidden">
              <CardHeader className="border-b border-white/[0.04] bg-white/[0.01] px-6 py-5">
              ...
              <CardContent className="p-6 space-y-6">
                {['Stock Up', 'Reduce', 'Maintain'].map(action => {
                  ...
                  const cfg = {
                    'Stock Up': { cls: 'border-green-500/20 bg-green-500/5', titleCls: 'text-green-400', bullet: 'bg-green-400' },
                    'Reduce': { cls: 'border-red-500/20 bg-red-500/5', titleCls: 'text-rose-400', bullet: 'bg-rose-400' },
                    'Maintain': { cls: 'border-white/[0.08] bg-white/[0.01]', titleCls: 'text-slate-400', bullet: 'bg-slate-400' }
                  };
                  ...
                  return (
                    <div key={action} className={`p-5 rounded-2xl border ${cls} backdrop-blur-md`}>
                      ...
                      <ul className="space-y-3 font-mono text-xs text-slate-300">
                        {list.map(d => (
                          <li key={d.product} className="flex items-start bg-black/25 border border-white/[0.03] p-3.5 rounded-xl gap-2 hover:bg-black/45 transition-colors">
                            <span className="text-[#00B4D8] font-bold shrink-0">{d.product}:</span>
                            <span className="text-slate-400">{d.advice}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
          ...
          <TabsContent value="actions" className="space-y-6 animate-fade-in-up">
            {/* Control Bar */}
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white/[0.02] border border-white/[0.08] p-4 rounded-2xl backdrop-blur-xl">
              <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                <div className="relative min-w-[200px] flex-1 sm:flex-initial">
                  ...
                  <input
                    ...
                    className="pl-10 pr-4 py-2.5 w-full bg-white/[0.02] border border-white/[0.08] rounded-xl text-slate-300 focus:outline-none focus:border-[#00B4D8]/40 focus:ring-1 focus:ring-[#00B4D8]/40 text-sm font-mono"
                  />
                </div>
                    ...
                    <button
                      ...
                      className={`px-3.5 py-2 font-mono text-[10px] uppercase tracking-wider rounded-xl transition-all border ${
                        actionFilter === f
                          ...
                          : 'bg-white/[0.01] border-white/[0.08] text-slate-400 hover:bg-white/[0.04] hover:text-white'
                      }`}
                    >
                      {f === 'all' ? 'All Rows' : f}
                    </button>
              ...
              <div className="flex items-center space-x-2.5 w-full lg:w-auto justify-end">
                <Button
                  ...
                  disabled={refreshing}
                  className="bg-gradient-to-r from-[#00B4D8] to-[#7B2FBE] hover:from-[#00B4D8]/90 hover:to-[#7B2FBE]/90 font-mono text-xs uppercase tracking-wider rounded-xl px-4 py-2.5 border-0 shadow-lg shrink-0"
                >
                ...
                <Button
                  variant="outline"
                  onClick={exportCsv}
                  className="border border-white/[0.08] bg-white/[0.02] hover:bg-white/[0.05] text-slate-300 hover:text-white font-mono text-xs uppercase tracking-wider rounded-xl px-4 py-2.5 shrink-0"
                >
              ...
              <CardContent className="p-0">
                <div className="overflow-x-auto">
                  <table className="min-w-full text-xs font-mono">
                    <thead>
                      <tr className="text-left text-slate-500 bg-white/[0.01] border-b border-white/[0.05]">
                        {[
                          ...
                          <th
                            key={key}
                            className="py-4 px-5 font-semibold cursor-pointer hover:text-white select-none transition-colors"
                    ...
                    <tbody className="divide-y divide-white/[0.04]">
                      {filtered.map(d => {
                        ...
                        return (
                          <tr
                            key={d.product}
                            className={`border-l-2 border-l-transparent transition-all hover:bg-white/[0.02] ${hoverAccent}`}
                          >
                            <td className="py-4 px-5 font-bold text-white">{d.product}</td>
                            <td className="py-4 px-5 text-slate-400">{Math.round(d.current).toLocaleString()}</td>
                            <td className="py-4 px-5 text-[#00B4D8] font-semibold">{Math.round(d.predicted).toLocaleString()}</td>
                            <td className={`py-4 px-5 font-bold ${d.delta >= 0 ? 'text-green-400' : 'text-rose-400'}`}>
                              {d.delta >= 0 ? '+' : ''}{Math.round(d.delta).toLocaleString()}
                            </td>
                            <td className="py-4 px-5 text-slate-400">{d.ratio.toFixed(2)}</td>
