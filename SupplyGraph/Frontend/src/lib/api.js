const API_BASE = process.env.REACT_APP_API_BASE !== undefined ? process.env.REACT_APP_API_BASE : "http://localhost:5000";
...
/** Trigger the AI multi-agent audit to run safety checks and supplier negotiations */
export async function triggerAgentAudit(companyId) {
  const res = await apiFetch(`${API_BASE}/api/reorder/proposals/${companyId}/trigger`, {
    method: 'POST',
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.error || `Agent audit failed (${res.status})`);
  }
  return res.json();
}

/** Fetch active procurement proposals generated by the agent system */
export async function getAgentProposals(companyId) {
  const res = await apiFetch(`${API_BASE}/api/reorder/proposals/${companyId}`);
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.error || `Fetch proposals failed (${res.status})`);
  }
  return res.json();
}

/** Approve or reject an active proposal */
export async function handleProposalAction(companyId, productId, action) {
  const res = await apiFetch(`${API_BASE}/api/reorder/proposals/${companyId}/${productId}/action`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.error || `Proposal action failed (${res.status})`);
  }
  return res.json();
}
